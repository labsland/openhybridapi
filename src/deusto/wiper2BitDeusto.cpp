...
void Wiper2BitDeustoSimulation::update(double delta) {
    ...
    this->log() << std::endl << "Move: " << mState.move1 << "," << mState.move2 << std::endl;

    if (requestWasRead) {
        this->log() << "Input:" << std::endl
                    << " RainSensor: " << request.rainSensor
                    << "; RightSensor: " << request.rightSensor
                    << "; LeftSensor: " << request.leftSensor << std::endl;
